...
/**
 * Fast group-theoretic + shallow solve-and-verify checks for 2x2 and 3x3 - no exhaustive search,
 * runs in well under a second. Shared by both `test/cube-fast.test.ts` (part of the default
 * `npm test`) and `test/cube.test.ts` (the full suite behind `npm run test:cube`, which adds the
 * slow exhaustive 2x2 state-count proof) so the two can't silently drift apart.
 */
export function runFastCubeChecks(assert: Assert) {
  // ---------------------------------------------------------------------------
  // 2x2 (exhaustively verified separately - this is the size the app defaults to)
  // ---------------------------------------------------------------------------
